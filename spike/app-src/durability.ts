/**
 * Capture durability — implements docs/CAPTURE-DURABILITY-ARCH-v1-CODEX.md.
 *
 * ARCHITECTURE IS CODEX'S (#13/#14). This file is a literal implementation.
 ...
 *   Media, capture_outbox, ps_crud and PowerSync projections mean NOTHING
 *   about commitment.
 */
...
/** Kill boundaries. Spec §5. K0-K7 = capture-boundary suite (no networking). */
...
/**
 * The failpoint contract (spec §5): pause the capture thread, publish
 * {trialNonce, captureId, boundary}, and WAIT. The harness must observe that
 * exact tuple before it terminates the process. A failpoint that does not
 * block would let the harness kill at the wrong moment and silently void the
 * boundary it thinks it tested.
 */
export type FailpointState = {
  armed: Boundary | null;
  trialNonce: string | null;
  reached: { trialNonce: string; captureId: string; boundary: Boundary } | null;
  /** Set while parked at the armed boundary; never resolves — the harness kills us. */
  parked: boolean;
};

export const failpoint: FailpointState = {
  armed: null,
  trialNonce: null,
  reached: null,
  parked: false,
...
async function hit(b: Boundary, captureId: string): Promise<void> {
  if (failpoint.armed !== b) return;
  failpoint.reached = { trialNonce: failpoint.trialNonce!, captureId, boundary: b };
  failpoint.parked = true;
  // Park forever. The status writer publishes `reached`; the harness sees the
  // exact tuple and issues simctl terminate. We never come back.
...
export type SavedEvent = { captureId: string; trialNonce: string | null; atMs: number };
...
export async function performCapture(
  ...
  await hit('K0_FINAL_BYTES_WRITTEN', captureId);

  // Step 4 — the finalized-file barrier. (See deviation note above.)
  await hit('K1_FILE_BARRIER_RETURNED', captureId);
  ...
  await hit('K2_HASH_VERIFIED', captureId);
  ...
  await hit('K3_MEDIA_INSTALLED', captureId);   // capture is now PREPARED
  ...
  try {
    await db.writeTransaction(async (tx) => {
      ...
      await hit('K4_COMMIT_ROW_INSERTED', captureId);
      ...
      await hit('K5_OUTBOX_ROW_INSERTED', captureId);
    });
  } catch (e: any) {
    return { ok: false, reason: `commit failed: ${e?.message ?? e}` };
  }
  await hit('K6_SQLITE_COMMIT_RETURNED', captureId);

  // Step 9 — ONLY NOW.
  savedEvents.push({ captureId, trialNonce: opts.trialNonce ?? null, atMs: Date.now() });
  await hit('K7_SAVED_EMITTED', captureId);
