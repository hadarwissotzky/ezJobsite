...
- **REQ-CAP5 — Save confirmation, audible + visual — fires ONLY at `COMMITTED` (a SQLite-derived state).** Success is confirmed on-screen **and** audibly/haptically; failure is loud, never silent. The confirmation is gated on the **capture-commit state machine** (`DURABILITY-DESIGN-v1.md` Artifact 1) reaching **`COMMITTED`** — media finalized + barriered + **verified** + **atomically installed**, AND the local SQLite transaction (Capture + Attachment, with PowerSync's `ps_crud` entries in that same transaction) **committed under `synchronous=FULL` / `F_FULLFSYNC`** (`DURABILITY-DESIGN-v1` §1.0 — *a returned COMMIT is not durable without it*). **Not** on the raw local write, not on a journal boolean, not on a manifest state, not on upload. `[trace: CAP-5; appstore "confirm it saved"; gated per Codex #6 C1 — a confirm on the local write is the phantom-"saved" bug]`

  > **⚠️ REWORDED 2026-07-16 (Codex #11 HIGH 12).** This requirement previously named **`MEDIA_COMMITTED`**, *"the manifest state"*. **That state no longer exists and must not be recreated.** It was the co-committer bug: it asked the manifest — a filesystem artifact — to record commitment, which only SQLite can know. **An implementer following the old wording would rebuild v1's exact defect.** `COMMITTED` is **derived from SQLite alone**; the manifest's terminal state is `VERIFIED` and it never records commitment. **`MEDIA_COMMITTED` must not appear in any manifest requirement anywhere.**
  - Accept: "saved ✓" + sound/haptic emits **only** for a capture whose manifest state is **`VERIFIED`** *(not `MEDIA_COMMITTED` — the manifest never records commitment; see the note under REQ-CAP8)*, **whose rows are committed in SQLite under `synchronous=FULL`**, and which passes the full oracle (decodable, duration credible, **hash recomputed from the bytes on disk** — never a stored-hash-to-stored-hash compare (Codex #9) — rows present, outbound mutation present); every failure surfaces non-silently. A "saved ✓" for a capture failing any oracle check is a **loss** — the worst-severity fault.
...
- **REQ-CAP8 — Write-ahead capture journal (the ezQuotePro fix).** At record **START** — before any media is finalized and before any network call — a durable **journal entry** is written recording that a capture began (id, project, author, start time, modality, intent). Media then streams into that record (REQ-CAP4). **The journal is a state machine, NOT a boolean `complete` flag** — the entry advances through the Artifact-1 manifest states (`STARTED` → `RECORDING` → `FINALIZING` → **`VERIFIED`**), each transition durable and idempotent, recorded in a **sidecar manifest** that is the recovery source of truth **for "were the bytes made"**, independent of SQLite. **`VERIFIED` is the manifest's terminal state.** A kill/crash **during** recording leaves a recoverable journal entry + whatever media was streamed (truncatable to the last chunk the log durably records). `[trace: CRITIC-REVIEW-05 — ezQuotePro wrote nothing on start, so a mid-recording kill lost everything; core-concept "commit before first network call"; boolean-complete banned per Codex #6 C1]`

  > **⚠️ `MEDIA_COMMITTED` REMOVED FROM THIS SEQUENCE 2026-07-16 (Codex #11 HIGH 12).** The manifest **must never record commitment** — that is the co-committer bug (`DURABILITY-DESIGN-v1` Artifact 1). The manifest answers *"were the bytes made, and where are they"*; **SQLite alone** answers *"is this capture committed"*. `COMMITTED` is a **SQLite-derived** state and appears in no manifest. Leaving the old sequence here would let an implementer rebuild the exact defect this document exists to prevent.
...
> **Build detail for the CompanyCam-parity + wedge push lives in `PRD-companycam-parity.md`** (2026-07-17). That doc *elevates* several seams here to buildable — the Feed (§7.1a/PM-D), Notifications (§7.1a), Change Order + Approval (§7.1/§7.2), collaborators — and adds net-new surfaces (photo grid/viewer, user tags, static map, bottom-nav, gallery/timeline share links, the structured-proposal review surface `REQ-PROC8`). This section stays the seam-level source; the PRD is authoritative for how they get built.
...
- **Project** *(was "Job")* — id, name, description, address/GPS + geofence, client ref, owner/tenant, **status** (`lead | in_progress | complete | archived` — expanded from `active/archived` per `PRD-companycam-parity.md §5.B`, hadar 2026-07-17; 2-state is the P1 fallback), **labels[]** (user color labels), recording_consent_state, member_ids[]. *(No `target_language` — killed with the "modular target" model; display language is per-user via `Member.preferred_language`; canonical is always English. — fixes critic M7.)*
