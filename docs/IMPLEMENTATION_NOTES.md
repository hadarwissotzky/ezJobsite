...
|---|---|---|---|
| 2026-07-23 | **Deleting an extra may never touch the iOS photo library** — discard removes only the app's own copies (`documentDirectory` files + bucket objects). Held today BY CONSTRUCTION: the app has no `expo-media-library` write permission, and the picker is read-only. Any future storage-cleanup or "free up space" feature must preserve this: gallery originals belong to the user, not the record. | The app's evidence copy of a gallery pick IS deleted with the extra (that is what discarding means); only the library original is out of bounds. | hadar |
...
**2026-07-22 — REAL Codex cross-check (local `codex review --base main`) of the record-screen action layer** (the c5 detail page: derived discussing chip, discussion log + reply composer on the record, state-dependent Remind/Revise/Send/Share/View-current bar, remind/revise dedupe in App.tsx). 2 P1, 2 P2 — all four accepted and FIXED in the same change:

- **P1 — lineage-wide messages inflated the derived status (FIXED).** The record derived `discussing` from `clientMessageCount(threadFor(...))`, but `threadFor` deliberately carries prior versions' messages, so a fresh revision opened as "Discussing" while the ledger said "Sent" — the exact chip disagreement R7 exists to prevent. Status now uses the ledger's own per-version signal (`questions[co.id]`, passed as `openQuestions`); the lineage thread still feeds the log and the answer/their-turn nuance. Same overcount removed from the remind gate.
- **P1 — `postReply` failures were swallowed (FIXED, both call sites).** It reports failure as `{ ok: false }`, not a throw; both the record's and the thread's onReply cleared the composer as if sent. Both now throw on `!ok`; ThreadScreen gained the same on-screen error note the record bar has.
- **P2 — "See the current version" only appeared on the authoring device (FIXED).** `supersededBy()` read only the local `co_supersession` intent row; a second device holds the lineage in the synced `change_order.superseded_by` column instead. The helper now falls back to the column.
- **P2 — stale layers while switching records (FIXED).** Opening a record over another (view-current, push tap) kept the prior record's summary/approval/thread/narration until each async read landed. `openRecord` now drops every layer the moment the new record row is set.
