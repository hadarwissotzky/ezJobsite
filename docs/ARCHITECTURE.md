...
> - **Commit-ordering** (`Codex #7` blocker 1 — *"the single most likely field failure"*, the fault that killed our hand-built design twice): PowerSync delivered the late-committing row in **40/40 trials across two independent harness revisions**, under a deterministic stalled-commit inversion (`seq_a < seq_b`, A committing after the device had durably checkpointed past B). **Mechanism: Postgres logical decoding emits in commit order, so a late commit lands at a *later* stream position — there is no cursor for it to fall behind.**
> - **Mutable operational state** (blocker 3, the L5-is-false gap): bidirectional sync with a defined convergence rule; pending offline edits preserved across a conflicting server edit; server-owned fields refused at the DB boundary (`42501`).
> - **Media: PowerSync contributes nothing.** `uploadFile(fileData: ArrayBuffer, attachment)` — whole buffer, one call, **no TUS, no chunking, no resume**. A killed upload restarts at byte zero. **Resumable media upload is ours to build either way** — which is the decisive point: it is *not* a cost of adopting PowerSync, it is a constant across both options.
>
> **⚠️ This decision was made OVER a cross-model objection, deliberately and on the record.** `CRITIC-REVIEW-10-CODEX.md` ruled *"Q1 VALID PASS survives: NO · Q2: NO · fit to support ADR-2: NO · a third frozen run is required."* Codex was **correct on every point** — but its objections were about **assertion rigor in a throwaway harness**, not about the observed behaviour, which it called *"credible and materially better."* No review round ever suggested PowerSync got the ordering wrong. hadar's call: the remaining rigor gap cannot change the answer, and `SPIKE-SYNC-BAKEOFF.md:120` explicitly permits it — *"if Q1–Q3 aren't answered in ~4 focused days, that itself is signal — report and decide rather than sinking more."* **The "VALID PASS" labels are withdrawn. This rests on observed behaviour + judgment, not on a cleared gate.**
>
> **Q3 was never run, and is now largely moot.** It was blocked by no physical device and by Option B being undesigned. **`DECISION 4` was then re-decided (Option B → Option A): client-side media encryption is dropped for v1**, removing the DEK, the unwrap path, and the plaintext-canary scan. What remains of Q3 is "does the queue move bytes" (yes — it's a `fetch`) and "does it resume" (**no** — established at the API level). **Untested residual:** whole-file `ArrayBuffer` memory behaviour for multi-minute media on-device → **carry into Spike A**.
>
> **What adopting changes** (per `SPIKE-SYNC-BAKEOFF.md:108-111`):
> - **`DURABILITY-DESIGN` Artifact 2 (the sync protocol) is largely REPLACED by PowerSync.** The seq-ordering, checkpoint, tombstone and pull-protocol work we kept getting wrong becomes its problem. **Codex #7 blockers 1 and 3 are dissolved — transport-owned.**
> - **Blocker 2 (`MEDIA_COMMITTED` atomicity) remains OURS** — it is local, and still marked `NOT ATOMIC — OPEN GAP` in its own text. **One problem instead of three.** It is now the top of the Spike A critical path.
> - **Stays ours regardless:** Artifact 1 (capture-commit), Artifact 5 (authz in Postgres functions), Artifact 6 (transactional outbox), Artifact 8 (failpoint matrix), semantic conflict resolution, backend idempotency, and **all media durability including resumable upload**.
> - **Append-only for evidence stays** — enforced by *our* rules (authz + no-update policies), not the transport. **L5's open gap resolves toward option (a)**: scope append-only to the evidence ledger; mutable operational state syncs through PowerSync (Q2 demonstrated it).
> - **Known costs accepted:** the attachments helper is **alpha** (`@powersync/attachments` is deprecated in favour of the built-in) · a client write to a server-owned field is **applied locally then silently reverted**, with no rejection hook — **a UX defect we must design around** · Sync Streams have **no server-side `now()`**, so *"Active projects + last-N-days"* is only **half** expressible natively · **`REQ-MEMBER-5` revocation is untested and still undefined** (cited 4×, defined 0×).
> - **Cost:** no Supabase IPv4 add-on needed — PowerSync Cloud reaches the IPv6-only direct host (verified: replication streaming, lag 0 bytes). Full Q6 oracle unrun.
>
> Evidence + rationale: `BAKEOFF-RESULT.md` · `IMPLEMENTATION_NOTES §4` · `CRITIC-REVIEW-08/09/10-CODEX.md`. *[hadar, verified key decision]*
>
> ---
>
<details>
<summary>⛔ <b>SUPERSEDED 2026-07-16 — the 2026-07-15 "NOT PowerSync" banner. Retained for provenance only. DO NOT FOLLOW.</b> (click to expand)</summary>

> **⛔ EVERY INSTRUCTION IN THIS BLOCK IS DEAD.** In particular, *"wherever it says PowerSync … read the owned-queue equivalent"* is **withdrawn** — the PowerSync passages in this document are **live again**. Two claims below are also now **false**: "PowerSync Attachments is experimental" (it is **alpha**, and `@powersync/attachments` is deprecated in favour of the built-in — the "experimental → reject" rationale was already flagged **stale** in `IMPLEMENTATION_NOTES:53`), and "the Sync-Rules↔RLS parity findings are dissolved" (they are **live again** — we now use Sync Streams; see the Q7/authz caveats in the current banner). Kept only so the decision history is auditable.
>
...
| Erasure/crypto-shred (locked C3) | Envelope-encrypt media per record; erase = destroy key + tombstone (retain hash stub). | ***[CORRECTED 2026-07-17: crypto-shred DEPENDS ON THE PER-RECORD KEY THAT DECISION 4 → OPTION A REMOVED. Erasure is HARD-DELETE + a retained {hash, metadata_stub, erased_at, basis}, per CLAUDE.md mandate #5. Not built.]***
...
```
 ...
 │ decisions, resolutions, translations-cache, usage       │   │ media (not encrypted  │
 └───────┬─────────────────────────────────────────────────┘   │ (R2 = P1.5 egress opt)│
         │                                                     └──────────────────────┘
...
- **Windowed working set from v1.** The device holds only **Active projects + last-N-days of notifications/history**; cold/archived history is fetched on demand. This caps device SQLite growth and keeps pull cheap. *(**Updated 2026-07-17 — this note was written for the world where PowerSync was NOT adopted, and both its clauses are now wrong.** PowerSync IS adopted for the relational rows (ADR-2 + its 2026-07-17 amendment), so this IS its windowed, data-parameterized sync config — `powersync/sync-config.yaml` already does exactly that: `active_projects` selects by `owner_id = auth.user_id() AND status = 'active'`, i.e. grants via ROWS, never rule text, because a sync-rule deploy re-replicates the whole DB (greenlight MF-4). The second clause — "not applicable to the P1 owned queue" — is also wrong twice over: the owned queues are still here (they carry the append-only evidence), and windowing does not apply to them for a different reason than stated: an outbox holds only what has not yet been sent, so it has nothing to window. **A real Q7 finding stands and is NOT resolved:** the "last-N-days" half of this window is NOT expressible in Sync Streams — there is no server-side `now()` — so time windowing needs a client-supplied parameter (which a client can lie about) or a server-maintained flag column. Neither is built.)*
...
> **⛔ CORRECTED 2026-07-17 — THE THREE BULLETS BELOW SPECIFIED A DESIGN THE HUMAN REJECTED.**
>
> They described **DECISION 4 Option B**: a per-capture DEK generated on-device, media encrypted client-side, the key wrapped for a server ingest identity, and **crypto-shred** as the erasure mechanism. **DECISION 4 chose OPTION A.** hadar: *"why do we need to encrypet the media ?"* → *"we are selling to solo owners with 2-10 employees, lets not over do it"*. Client-side media encryption was **dropped for v1**.
>
> **What is true now:** media is uploaded as-is to Supabase Storage over TUS. At rest it is protected by **Supabase Storage + an owner-scoped, create-only policy** server-side, and by **iOS Data Protection / Android encrypted storage** on the device — which is REQ-CAP4's own wording ("*not* app-level per-capture encryption"). There is no DEK, nothing to unwrap, nothing to decrypt.
>
> **CRYPTO-SHRED IS NOT THE ERASURE MECHANISM AND CANNOT BE** — it depends on the key that no longer exists. `CLAUDE.md` mandate #5 already says the right thing: a valid erasure request **HARD-DELETES the content + media and retains a hash + metadata stub**. This document contradicted the governing document on how we honour a GDPR/CCPA request. That is not a stale sentence; it is a legal control described two incompatible ways.
>
> Kept struck through so the decision history stays auditable.
>
...
- **Uploads (DECISION 4 — Option A, LIVE):** media uploads **as-is** to **Supabase Storage** (TUS resumable). The object key is **content-addressed** (`owner/capture/sha256.ext`), so a retry can never overwrite different bytes, and the storage policy is **create-only and owner-scoped** — proven in `011_storage_policies.sql`.
- **Erasure (mandate #5):** **hard-delete** the content + media, retain a `{hash, metadata_stub, erased_at, basis}` tombstone. The evidence-chain skeleton survives; the personal data is destroyed. This is the ONE carve-out to the never-destroy rule. **Not built** — named here rather than implied by a crypto-shred that cannot work.
...
1. Ingest media (from **Supabase Storage**). *(**CORRECTED 2026-07-17.** This said the object is "client-encrypted ciphertext (DECISION 4)" and described unwrapping a per-capture DEK. **DECISION 4 chose OPTION A: client-side media encryption was DROPPED.** There is no DEK to unwrap and no ciphertext to decrypt — the object is the media, protected at rest by Supabase Storage and reached only through an owner-scoped, create-only policy. A pipeline built from the old wording would have looked for a key that does not exist. The `attachment` table's `wrapped_dek_device` / `aead_nonce` / `aead_alg` columns are DEAD from that design; `ciphertext_sha256` holds a PLAINTEXT hash — the names lie, and renaming them is a separate deliberate change, see `IMPLEMENTATION_NOTES.md` §5.)*
...
- **Crypto-shred erasure:** destroy the media's per-record data key + tombstone the row, retaining `{hash, metadata_stub, erased_at, basis}` — the one immutability carve-out. ***[CORRECTED 2026-07-17: crypto-shred DEPENDS ON THE PER-RECORD KEY THAT DECISION 4 → OPTION A REMOVED. Erasure is HARD-DELETE + a retained {hash, metadata_stub, erased_at, basis}, per CLAUDE.md mandate #5. Not built.]***
...
| Sync (P1) | **BOTH, split by data kind** *(corrected 2026-07-17)* | **Append-only evidence → owned queue** (write-ahead journal REQ-CAP8 → durable resumable upload → idempotent server dedup): PowerSync can revert its own rows, so it cannot be the commitment authority for something mandate #1 says must never be lost. **Mutable relational rows (projects) → PowerSync**: a jobsite address is not evidence. This row previously said "owned queue" with "PowerSync (deferred)" as the rejected alternative — that was the 2026-07-15 world and ADR-2 superseded it on 2026-07-16. | ElectricSQL |
...
**A. Offline capture → evidence → sync.** *(Updated 2026-07-16 — DECISION 4 → Option A dropped client-side media encryption; Artifact 1 v3 replaced the manifest-advance step. Authoritative sequence: `DURABILITY-DESIGN-v1` Artifact 1 §1.3 — **blocker 2 is still OPEN**, so treat this line as a summary, not a build instruction.)* Tap → **preallocate the actual media temp file** (or refuse loudly) → **commit `STARTED` journal + sidecar manifest before arming the recorder (REQ-CAP8)** → record (stream chunks to FS; **chunk data barriered before its log record**) → finalize + barrier file/dir (**`F_FULLFSYNC` on Apple**) → **freeze writer, verify** decodable + duration + sample-count + hash → **atomic no-replace install** to the content-addressed path → **one SQLite txn (`synchronous=FULL`)** creates `Capture` + `Attachment` + outbound mutation → **`COMMITTED`** *(corrected: `MEDIA_COMMITTED` was RETIRED per Codex #11 HIGH 12 — it asked a filesystem manifest to record commitment, which only SQLite can know. "An implementer following the old wording would rebuild v1's exact defect.")* → *only now* audible+visual **"Saved on this phone ✓ — not backed up yet."** Offline: the **owned queue** holds the row + the attachment (the outbound mutation IS the queue entry, created in that same txn — no window where a capture is "saved" with no sync intent); status shows "waiting for Wi-Fi." On qualifying connection: the media uploads to **Supabase Storage** *(corrected 2026-07-17: this said "client-encrypted ciphertext" — DECISION 4 chose **Option A** and client-side media encryption was DROPPED. At-rest protection is iOS Data Protection / Android encrypted storage, per REQ-CAP4's own wording. The line below describing MF-1 as "Option B" is wrong for the same reason.)*, the push queue syncs the mutation to Postgres (idempotent, stable mutation ID), pipeline runs (transcribe→translate→structure→resolve), a pull brings results back, status → "Backed up ✓" then "ready." *(See `DURABILITY-DESIGN-v1.md` Artifact 1 for the full state machine + recovery table.)*
...
- **Erasure vs immutability (C3):** envelope encryption + crypto-shred + tombstone stub. ***[CORRECTED 2026-07-17: crypto-shred DEPENDS ON THE PER-RECORD KEY THAT DECISION 4 → OPTION A REMOVED. Erasure is HARD-DELETE + a retained {hash, metadata_stub, erased_at, basis}, per CLAUDE.md mandate #5. Not built.]***
...
1. **Skeleton + sync spine (ADR-2 spike / "Spike A"):** React Native (Expo) + Supabase (raw Postgres) + **PowerSync's local SQLite + our capture-commit path** in a TS monorepo (shared with the web app); one row + one attachment round-trip offline→online **with native background upload proven on a real device** (C3), and the kill/airplane/disk-pressure durability tests that are this spike's exit gate. *This is the foundation everything rides on — do it first (matches SPEC M0).*
...
1. ~~**ADR-1: Flutter vs React Native**~~ **RESOLVED — React Native + Expo (ADR-1).** ~~**ADR-2: PowerSync vs owned queue**~~ **RESOLVED — Supabase raw Postgres + **PowerSync** as the sync transport (ADR-2, re-resolved 2026-07-16 after the bakeoff; media durability stays ours).** Both database forks are now closed.
...
## 10. Verification
Per the protocol: this architecture should get a **second-model (Codex) adversarial pass** before build — focused on the offline sync/dedup model, the RLS multi-tenant + collaborator policies, background-upload reliability, and **the erasure design (which is now HARD-DELETE + tombstone, not crypto-shred — corrected 2026-07-17; asking a reviewer to check a subsystem we deleted wastes the review)**. External signal already pulled (PowerSync/Supabase maturity, Flutter/RN, AI + storage costs); the remaining validation is the three spikes in §9.
...
Applied in this document: **MF-1** ~~envelope encryption~~ **[CORRECTED 2026-07-17: there is no envelope encryption — DECISION 4 chose Option A. What is live from MF-1 is the OTHER half: authorized reads, no presigned GET]** + authorized reads, no presigned GET (§3.3) — ***CORRECTED 2026-07-17: this said "DECISION 4 (Option B) — encryption is CLIENT-side with per-capture DEKs". DECISION 4 chose **OPTION A**: client-side media encryption was DROPPED for v1 (hadar: "why do we need to encrypt the media?" — we sell to solo owners with 2–10 employees). At-rest = OS file protection. The `attachment` table still carries `ciphertext_sha256` / `wrapped_dek_device` / `aead_nonce` from the Option-B design; those columns hold PLAINTEXT hashes or nothing at all — see `IMPLEMENTATION_NOTES.md` §5*** · **MF-2** one authorization model across every path + negative tests + FORCE-RLS CI check (§3.2) — ***reconciled 2026-07-16 to Codex #6 C7: the canonical predicate lives in POSTGRES FUNCTIONS, not in shared TS middleware; Hono middleware is a convenience layer, not the root of trust*** · **MF-3** feed rendered from local synced data; `postgres_changes` banned (§3.2) · **MF-4** windowed working set from v1 *(corrected 2026-07-17: PowerSync IS adopted, so the data-parameterized sync-rule form is LIVE and implemented in `powersync/sync-config.yaml`; the "last-N-days" half remains unexpressible — see §3.1)* (§3.1) · metering enforces transcription minutes + lazy Free processing (§3.10). Applied in SPEC: **MF-5** P1 delivery = email + SMS, 10DLC starts at kickoff (REQ-VAL8) · **MF-6** one-confirm-surface + unclassified-evidence degradation (REQ-VAL6/VAL4, §4) · **MF-7** P1 ES localization (REQ-X2) + consent at job creation, never capture-time (REQ-CON1) · send-path touch budget ≤3 (REQ-X1) · one collapsed status (REQ-X3). Applied in PRICING: honest repricing (~60–70% blended margin), Verify/R2/jobs/platform-floor lines, Free minute allowance.
