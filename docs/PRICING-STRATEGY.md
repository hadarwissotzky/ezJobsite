...
Healthy SaaS margins even at aggressive prices — because the cost floor is cents. **The margin risk is SMS**, not AI: control it with per-tier SMS-signature allowances (overage or pass-through for heavy users) and in-app/push for everything that isn't a binding signature. Storage risk is now moot — **raw video is never retained or uploaded after successful extraction** (extracted on-device to audio + key stills; the raw file is an encrypted temporary asset held only until the derived assets are verified, then cleaned up — see `ARCHITECTURE.md §3.1`), so **cloud** storage stays negligible on every tier; object storage (Supabase Storage in P1) holds only the small audio/image footprint. *(The transient raw-video file consumes device storage during capture only — a device-capacity concern handled by the capture-commit reservation step, not a cloud cost.)* This keeps the free-user cost floor intact and means video can be a normal feature, not a paid-only one.
