...
// which would count a mention in a comment as wiring.
{
  ...
  const KNOWN_UNWIRED = {
    ...
    'clientsms.ts': 'Written and tested (16 cases) but NOT YET WIRED, and the reason is a hard boundary rather than a judgement: its only call site is the sentLink modal in App.tsx, which was off-limits to the session that wrote it. Until it is wired, the SMS that carries an approval link is still `${shownContent}\\n\\n${url}` — the entire frozen instrument, ~7 UCS-2 segments, link below the fold. The owed change is two lines and is written out verbatim in docs/CLIENT-PORTAL.md §"Owed". This entry is a debt, not an exemption: delete it the moment App.tsx calls clientSmsBody',
  };
