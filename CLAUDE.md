...
- **Backend/API + data (LOCKED 2026-07-15; sync superseded 2026-07-16/17):** device store = **SQLite (encrypted)**; cloud = **PostgreSQL via Supabase (raw Postgres)** — Auth, RLS, Edge Functions (AI pipeline), Storage for blobs. Firebase rejected (NoSQL, weaker relational/RLS).
  ...
  **✅ SIGNED OFF 2026-07-17 (hadar).** The rule is locked: **append-only evidence (captures, decisions, change orders, tags, comments) → owned outbox; mutable rows (project status/labels, membership, company) → PowerSync.** The trade-off (two transports = more surface) is accepted: a developer must know which rule applies before adding a table, and this is that rule. Any new table picks its transport by this one line. *(Reconciles the stale "owned queue, PowerSync deferred" language in `SPEC §0` line 15 — see the fix there; and `PRD-companycam-parity.md §6/§11.)*
...
| **`docs/CLIENT-PORTAL.md`** | **The counterparty-facing surface: what the SMS says, what the no-login page at the end of the link does (approve · communicate · negotiate), the token's security model and what an attacker holding a link can do, and what the contractor is told when the client acts. Read it before touching `apps/web/`, `confirmations.ts`, or anything anon can call. Owns none of the lifecycle — that is `SPEC-extra-lifecycle-v1.md`.** |
