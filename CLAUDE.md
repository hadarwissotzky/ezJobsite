...
1. **Never lose a capture.** Capture is **local-first**, written to durable storage *before* any network call, confirmed to the user **audibly and visually**, and recoverable after a crash/kill. Silent data loss is the single unforgivable sin in this category — every incumbent bleeds trust on it.
...
- **Backend/API + data (LOCKED 2026-07-15):** device store = **SQLite (encrypted)**; cloud = **PostgreSQL via Supabase (raw Postgres)** — Auth, RLS, Edge Functions (AI pipeline), Storage for blobs; relational fit for multi-tenant + collaborators; first-class TS/RN SDKs. **Sync for P1 = a simple owned queue, NOT PowerSync** — the three-layer durability we own (device write-ahead capture journal per REQ-CAP8 → durable resumable upload queue → reconnect processing queue). PowerSync deferred to **P1.5+**, adopted only if multi-device conflict-free *relational* sync (the collaborator feed) earns it. Rationale: the ezQuotePro post-mortem proved the failure mode is a cloud-first capture path, not a weak sync engine; an owned queue is fully debuggable by a solo dev, carries no experimental-API risk, and dissolves the PowerSync-specific review findings (Sync-Rules↔RLS parity, re-replication cost). Firebase rejected (NoSQL, weaker relational/RLS). *(Full rationale + trade-off: `IMPLEMENTATION_NOTES.md` §2, 2026-07-15 DATABASE LOCKED.)*
